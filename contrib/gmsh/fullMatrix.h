...
public:
  ...
  inline void set(int r, scalar v)
  {
    ...
    if(r >= _r || r < 0) {
      ...
      printf("Invalid index in vector: %i (size = %i)\n", r, _r);
      return;
    }
#endif
    (*this)(r) = v;
...
public:
  ...
  inline scalar get(int r, int c) const
  {
    ...
    if(r >= _r || r < 0 || c >= _c || c < 0) {
      // Msg::Error("Invalid index in dense matrix: %i %i (size = %i %i)", r, c, _r, _c);
      printf("Invalid index in dense matrix: %i %i (size = %i %i)\n", r, c, _r, _c);
      return 0;
    }
#endif
    return (*this)(r, c);
  ...
  void reshape(int nbRows, int nbColumns)
  {
    ...
    if(nbRows * nbColumns != _r * _c)
      ...
      printf("Invalid dense matrix reshape: total number of entries must "
                 "be equal (new %i x %i != old %i x %i)\n",
                 nbRows, nbColumns, _r, _c);
    _r = nbRows;
    _c = nbColumns;
  ...
  void operator+=(const fullMatrix<scalar> &other)
  {
    if(_r != other._r || _c != other._c) {
      ...
      printf("Cannot sum dense matrices of different sizes\n");
      return;
    }
    for(int i = 0; i < _r * _c; ++i) _data[i] += other._data[i];
  }
  inline scalar operator()(int i, int j) const
  {
    ...
    if(i >= _r || i < 0 || j >= _c || j < 0) {
      // Msg::Error("Invalid index to access dense matrix: %i %i (size = %i %i)", i, j, _r, _c);
  ...
  void copy(const fullMatrix<scalar> &a)
  {
    if(_data && !_ownData) {
      ...
      //            "instead");
      printf("Dense matrix copy prohibited for proxies, use setAll "
                 "instead\n");
      return;
    }
    if(_r != a._r || _c != a._c) {
...
#elif !defined(HAVE_BLAS)
  {
    if(_r != m._r || _c != m._c) {
      ...
      printf("Dense matrix sizes do not match in setAll\n");
      return;
    }
  ...
  inline void transposeInPlace()
  {
    if(_r != _c) {
      ...
      //            "(size = %d %d)", _r, _c);
      printf("In-place transposition requires a square matrix "
                 "(size = %d %d)\n", _r, _c);
      return;
    }
    scalar t;
...
#elif !defined(HAVE_LAPACK)
  {
    ...
    printf("LU factorization and substitution requires Eigen or LAPACK\n");
    return false;
  }
#endif
  ;
  ...
  {
    if(_r != _c) {
      // Msg::Error("Dense matrix inverse requires square matrix (size = %d %d)", _r, _c);
...
#elif !defined(HAVE_LAPACK)
  {
    ...
    printf("Matrix inversion requires Eigen or LAPACK\n");
    return false;
  }
#endif
  ;
  ...
  {
    if(_r != _c) {
      // Msg::Error("Dense matrix inversion requires square matrix (size = %d %d)", _r, _c);
      printf("Dense matrix inversion requires square matrix (size = %d %d)\n", _r, _c);
      return false;
    }
    EigenMat ma(_data, _r, _c);
    ma = ma.inverse();
...
#elif !defined(HAVE_LAPACK)
  {
    ...
    //            "LAPACK");
    printf("Eigenvalue computation of dense matrices requires Eigen or "
               "LAPACK\n");
    return false;
  }
#endif
...
#elif !defined(HAVE_LAPACK)
  {
    ...
    //            "Eigen or LAPACK");
    printf("Singular value decomposition of dense matrices requires "
               "Eigen or LAPACK\n");
    return false;
  }
#endif
