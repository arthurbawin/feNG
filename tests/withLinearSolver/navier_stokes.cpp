...
/*
  Tests the implementation of mixed vector FE spaces and the Navier-Stokes equations
...
namespace navier_stokes {
  ...
  namespace MMS {
    ...
    void uSrc_f(const feFunctionArguments &args, const std::vector<double> &/*par*/, std::vector<double> &res)
    {
      ...
      const double u[2] = {x*x*x*x*y*y*y*y, (-4./5. * x*x*x*y*y*y*y*y)};
      // Convention: defined as gradu_ij = du_j/dx_i to agree with directional derivatives
      const double gradu[2][2] = {{4.*x*x*x*y*y*y*y, -12.*x*x*y*y*y*y*y/5.},
                                  {4.*x*x*x*x*y*y*y,  -4.*x*x*x*y*y*y*y}};
      const double uDotGradu[2] = {u[0]*gradu[0][0] + u[1]*gradu[1][0],
                                   u[0]*gradu[0][1] + u[1]*gradu[1][1]};
                                   
      res[0] = - (- uDotGradu[0] + minus_dpdx[0] + lap_u[0]);
      res[1] = - (- uDotGradu[1] + minus_dpdx[1] + lap_u[1]);
    ...
    int meshConvergence(std::stringstream &resultBuffer, int order, int numMeshes, int degreeQuadrature)
    {
      ...
      for(int i = 0; i < numMeshes; ++i)
      {
        ...
        navier_stokes::MMS::solve(meshFile, order, degreeQuadrature, NLoptions, nElm[i], errU[i], errP[i], divergenceFormulation);
      }
  ...
  namespace poiseuille {
    ...
    int meshConvergence(int order, int degreeQuadrature, double errors[4])
    {
      feNLSolverOptions NLoptions{1e-10, 1e-10, 1e4, 10, 4, 1e-1};
      ...
      bool divergenceFormulation = true;
      navier_stokes::poiseuille::solve(meshFile, order, degreeQuadrature, NLoptions, nElm, errU, errP, divergenceFormulation);
      std::cout << "Taylor-Hood elements P" << order << "-P" << order-1 << std::endl;
      std::cout << "Navier-Stokes Poiseuille - Divergence formulation - Error on u" << std::endl;
      std::cout
        << std::scientific
        ...
        << std::endl;
      std::cout << "Navier-Stokes Poiseuille - Divergence formulation - Error on p" << std::endl;
      std::cout
        << std::scientific
        << std::setw(16) << std::right
        << "nElm = " << std::setprecision(6) << nElm << std::setw(16) << std::right
      ...
      navier_stokes::poiseuille::solve(meshFile, order, degreeQuadrature, NLoptions, nElm, errU, errP, divergenceFormulation);
      std::cout << "Taylor-Hood elements P" << order << "-P" << order-1 << std::endl;
      std::cout << "Navier-Stokes Poiseuille - Laplacian formulation - Error on u" << std::endl;
      std::cout
        << std::scientific
        << std::setw(16) << std::right
        << "nElm = " << std::setprecision(6) << nElm << std::setw(16) << std::right
        << "errU = " << std::setprecision(6) << errU << std::setw(16) << std::right
        << std::endl;
      std::cout << "Navier-Stokes Poiseuille - Laplacian formulation - Error on p" << std::endl;
      std::cout
        << std::scientific
        << std::setw(16) << std::right
        << "nElm = " << std::setprecision(6) << nElm << std::setw(16) << std::right
...
TEST(NavierStokes, MMS)
{
  ...
  navier_stokes::MMS::meshConvergence(resultBuffer, 2, 4, degreeQuadrature);
  // navier_stokes::MMS::meshConvergence(resultBuffer, 3, 4, degreeQuadrature);
  // navier_stokes::MMS::meshConvergence(resultBuffer, 4, 4, degreeQuadrature);
...
TEST(NavierStokes, Poiseuille)
{
  ...
  int degreeQuadrature = 8;
  double errors[4];
  navier_stokes::poiseuille::meshConvergence(2, degreeQuadrature, errors);
  for(int i = 0; i < 4; ++i) ASSERT_TRUE(errors[i] < error_tolerance);
  // navier_stokes::poiseuille::meshConvergence(3, degreeQuadrature, errors);
  // for(int i = 0; i < 4; ++i) ASSERT_TRUE(errors[i] < error_tolerance);
  // navier_stokes::poiseuille::meshConvergence(4, degreeQuadrature, errors);
  // for(int i = 0; i < 4; ++i) ASSERT_TRUE(errors[i] < error_tolerance);
