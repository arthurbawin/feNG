...
double fSolDotDot(const double t, const std::vector<double> &x, const std::vector<double> par) {
  double x1 = x[0];
  return 30*(x1*x1-25)*pow(t,4) + 30*x1*x1*pow(t,4);
}
...
int main(int argc, char **argv)
{
  ...
  for(int iter = 0; iter < nIter; ++iter) {
    ...
    int dim;
    feSpace *U_BXA, *U_BXB, *U_M1D, *L_BXA, *L_BXB, *V_BXA, *V_BXB, *W_BXA, *W_BXB;
    ...
    feCheck(createFiniteElementSpace(W_BXA, &mesh, dim=0, POINT, 0, "W", "BXA", degreeQuadrature, funSol));
    feCheck(createFiniteElementSpace(W_BXB, &mesh, dim=0, POINT, 0, "W", "BXB", degreeQuadrature, funSolDot));
    ...
    feBilinearForm weakBC_A(spaceWeak_A, &mesh, degreeQuadrature, new feSysElm_0D_weakBC_edo2(1.0, funSolDotDot));
    feBilinearForm weakBC_B(spaceWeak_B, &mesh, degreeQuadrature, new feSysElm_0D_weakBC_edo2(1.0, funSolDotDot));
    ...
    feCheck(createTimeIntegrator(solver, DC5F, tol, system, &metaNumber, &sol, &mesh, comput, exportData, t0, t1, nTimeSteps, CodeIni));
