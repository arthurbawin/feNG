...
//   double x1 = x[0];
//   return -16*sin(4*t)*exp(2*x1) ;
// }

double fSol(const double t, const std::vector<double> &x, const std::vector<double> par) {
  double x1 = x[0];
  return pow(t, 1) * x1 * (x1 - 5.) + x1 + 1.;
}

double fSource(const double t, const std::vector<double> &x, const std::vector<double> par) {
  double x1 = x[0];
  double c1 = par[0];
  return -1. * pow(t, 0) * x1 * (x1 - 5.) + c1 * 2 * pow(t, 1);
}

double flambda_A(const double t, const std::vector<double> &x, const std::vector<double> par) {
  double x1 = x[0];
  double c1 = par[0];
  return c1 * (pow(t, 1) * (2 * x1 - 5.) + 1.);
}
double flambda_B(const double t, const std::vector<double> &x, const std::vector<double> par) {
  double x1 = x[0];
  double c1 = par[0];
...
double fSolDot(const double t, const std::vector<double> &x, const std::vector<double> par) {
  double x1 = x[0];
  return 1. * pow(t, 0) * x1 * (x1 - 5.);
}

double fSolDotDot(const double t, const std::vector<double> &x, const std::vector<double> par) {
  double x1 = x[0];
  // return 2.*pow(t, 0)*x1*(x1-5.) ;
  return 0.;
}
...
int main(int argc, char **argv) {
  ...
  for(int iter = 0; iter < nIter; ++iter) {
    ...
    // Formes (bi)lineaires
    int degQuad = 9; // TODO : change to deg
    ...
    feBilinearForm *weakBC_U_A = new feBilinearForm(spaceWeak_A, mesh, degQuad,
                                                    new feSysElm_0D_weakBC_edo2(1.0, funSolDotDot));
    feBilinearForm *weakBC_U_B = new feBilinearForm(spaceWeak_B, mesh, degQuad,
                                                    new feSysElm_0D_weakBC_edo2(1.0, funSolDotDot));
    ...
    feNorm *normBDF1 = new feNorm(&U_M1D, mesh, degQuad, funSol);
    feNorm *normDC2F = new feNorm(&U_M1D, mesh, degQuad, funSol);
    feNorm *normDC3F = new feNorm(&U_M1D, mesh, degQuad, funSol);
    feNorm *normBDF1Lambda_A = new feNorm(&L_BXA, mesh, degQuad, funLambda_A);
    feNorm *normBDF1Lambda_B = new feNorm(&L_BXB, mesh, degQuad, funLambda_B);
    feNorm *normDC2FLambda_A = new feNorm(&L_BXA, mesh, degQuad, funLambda_A);
    feNorm *normDC2FLambda_B = new feNorm(&L_BXB, mesh, degQuad, funLambda_B);
    feNorm *normDC3FLambda_A = new feNorm(&L_BXA, mesh, degQuad, funLambda_A);
    feNorm *normDC3FLambda_B = new feNorm(&L_BXB, mesh, degQuad, funLambda_B);
