...
int main(int argc, char **argv) {
  ...
  feCheck(createFiniteElementSpace(uDomaine, &mesh, dim = 2, TRI,  order, "U", "Domaine", degreeQuadrature, funSol));
  ...
  // Define post-processing tools to compute norms and whatnot (norms will be replaced by fePostProc)
  ...
  // Solve the discrete problem. Initialize a TimeIntegrator object and tolerances on the Newton-Raphson 
  // nonlinear solver (tolerance on the solution correction dx, tolerance on the residual, max number 
  // of iterations). Here the PDE is linear : the nonlinear solver should converge in 2 iterations.
  // The TimeIntegrator can be STATIONARY, BDF1, BDF2 or a deferred-correction method (DC2F, DC3, DC3F).
  // The solution will be exported for visualization according to the exportData structure.
  // The linear system is assembled and solved in the "makeSteps()" call.
