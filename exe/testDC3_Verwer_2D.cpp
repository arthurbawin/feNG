...
//   double a = par[1];
//   double beta = (a == 0.) ? 0. : a * pow(t, a - 1.);
//   return -beta * sin(x1*(2*M_PI))* cos(y1*(2*M_PI)) - c1 *2*pow(t, a) * sin(x1*(2*M_PI))*
//   cos(y1*(2*M_PI))*((2*M_PI)*(2*M_PI));
// }
...
// double flambda_gauche(const double t, const std::vector<double> &x, const std::vector<double>
// par) {
//   double x1 = x[0];
//   double y1 = x[1];
//   double c1 = par[0];
...
int main(int argc, char **argv) {
  ...
  for(int iter = 0; iter < nIter; ++iter) {
    ...
    std::vector<feSpace *> fespace = {&U_angle, &U_surface, &U_haut,  &U_gauche, &L_angle,
                                      &L_haut,  &L_gauche,  &V_angle, &V_haut,   &V_gauche};
    ...
    feBilinearForm *weakBC_U_angle = new feBilinearForm(
      spaceWeak_angle, mesh, degQuad, new feSysElm_1D_weakBC_edo1(1.0, funSolDot));
    feBilinearForm *weakBC_U_haut = new feBilinearForm(spaceWeak_haut, mesh, degQuad,
                                                       new feSysElm_1D_weakBC_edo1(1.0, funSolDot));
    feBilinearForm *weakBC_U_gauche = new feBilinearForm(
      spaceWeak_gauche, mesh, degQuad, new feSysElm_1D_weakBC_edo1(1.0, funSolDot));

    std::vector<feBilinearForm *> formMatrices = {diff_U_M2D, masse_U_M2D, weakBC_U_angle,
                                                  weakBC_U_haut, weakBC_U_gauche};
    std::vector<feBilinearForm *> formResiduals = {diff_U_M2D,     masse_U_M2D,   source_U_M2D,
                                                   weakBC_U_angle, weakBC_U_haut, weakBC_U_gauche};
    ...
    std::vector<feNorm *> norms = {normBDF2,
                                   normDC3,
                                   normBDF2Lambda_angle,
                                   normBDF2Lambda_gauche,
                                   normDC3Lambda_angle,
                                   normDC3Lambda_gauche};
    ...
    normDC3_angle[2 * iter] =
      *std::max_element(normL2_DC3_Lambda_angle.begin(), normL2_DC3_Lambda_angle.end());
    normDC3_gauche[2 * iter] =
      *std::max_element(normL2_DC3_Lambda_gauche.begin(), normL2_DC3_Lambda_gauche.end());
#endif
  ...
  for(int i = 1; i < nIter; ++i) {
    ...
    normBDF2_gauche[2 * i + 1] =
      log(normBDF2_gauche[2 * (i - 1)] / normBDF2_gauche[2 * i]) / log(2.);
  ...
  printf("%12s \t %12s \t %16s \t %16s \t %16s \t %16s \n", "nTimeSteps", "nElm",
         "||E_lambda_angle||", "Taux lambda_angle", "||E_lambda_gauche||", "Taux lambda_gauche");
  for(int i = 0; i < nIter; ++i)
    printf("%12d \t %12d \t %16.6e \t %16.6e \t %16.6e \t %16.6e  \n", TT[i], nElm[i],
           normBDF2_angle[2 * i], normBDF2_angle[2 * i + 1], normBDF2_gauche[2 * i],
           normBDF2_gauche[2 * i + 1]);
  printf("\n");
  printf("\n");
  ...
  for(int i = 0; i < nIter; ++i)
    printf("%12d \t %12d \t %16.6e \t %16.6e \t %16.6e \t %16.6e  \n", TT[i], nElm[i],
           normDC3_angle[2 * i], normDC3_angle[2 * i + 1], normDC3_gauche[2 * i],
           normDC3_gauche[2 * i + 1]);
#ifdef USING_PETSC
  petscFinalize();
#endif
  return 0;
