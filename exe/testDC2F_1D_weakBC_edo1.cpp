...
int main(int argc, char **argv) {
  ...
  for(int iter = 0; iter < nIter; ++iter) {
    ...
    TT[iter] = nTimeSteps;
    feSolution *solDC2F = new feSolution(mesh, fespace, feEssBC, metaNumber);
    ...
    feBilinearForm *weakBC_U_A =
      new feBilinearForm(spaceWeak_A, mesh, degQuad, new feSysElm_0D_weakBC_edo1(1.0, funSolDot));
    feBilinearForm *weakBC_U_B =
      new feBilinearForm(spaceWeak_B, mesh, degQuad, new feSysElm_0D_weakBC_edo1(1.0, funSolDot));
    ...
    std::vector<feNorm *> norms = {normBDF1,         normDC2F,         normBDF1Lambda_A,
                                   normBDF1Lambda_B, normDC2FLambda_A, normDC2FLambda_B};
    // Systeme lineaire
    ...
    DC2FSolver solver(tol, metaNumber, linearSystem, solDC2F, norms, mesh, t0, t1, nTimeSteps);
