...
int main(int argc, char **argv) {
  ...
  for(int iter = 0; iter < nIter; ++iter) {
    std::string meshName = "../../data/Square/SquareNS_1Edge/squareNS" + std::to_string(iter + 1) + "_1Edge.msh";
    ...
    std::vector<feSpace *> fespace = {&U_Bord, &U_surface, &V_Bord, &V_surface, &P_surface, &Lu_Bord, &Lv_Bord,&Bu_Bord, &Bv_Bord};
    ...
    feBilinearForm *weakBC_bordU = new feBilinearForm(spaceWeak_bordU, mesh, nQuad,
                                                     new feSysElm_1D_weakBC_edo1(1.0, funUDot));
    feBilinearForm *weakBC_bordV = new feBilinearForm(spaceWeak_bordV, mesh, nQuad,
                                                     new feSysElm_1D_weakBC_edo1(1.0, funVDot));
    ...
    feNorm *normVecVeloc_BDF2 = new feNorm(fespaceNorm, mesh, nQuad,funZero,funVecVeloc, "NormL2");
    ...
    std::vector<feNorm *> norms = { normVecVeloc_BDF2,normVecVeloc_DC3, normPression_BDF2, normPression_DC3, normLambda_BDF2, normLambda_DC3, IntLambda_BDF2, IntLambda_DC3 };
    // Systeme lineaire
    ...
    for(int i = 0; i < normLDC3.size(); i++)
      normLDC3[i] = fabs(normLDC3[i]);
    for(int i = 0; i < IntLBDF2.size(); i++){
      // std::cout<<IntLBDF2[i]<<std::endl;
      IntLBDF2[i] = fabs(IntLBDF2[i]);}
    for(int i = 0; i < IntLDC3.size(); i++)
      IntLDC3[i] = fabs(IntLDC3[i]);
    for(int i = 0; i < normUBDF2.size(); i++){
      std::cout<<"U_BDF2  "<<normUBDF2[i]<<std::endl;
    }
    for(int i = 0; i < normUDC3.size(); i++){
      std::cout<<"U_DC3  "<<normUDC3[i]<<std::endl;
    }
    for(int i = 0; i < normLBDF2.size(); i++){
      std::cout<<"L_BDF2  "<<normLBDF2[i]<<std::endl;
    }
    for(int i = 0; i < normLDC3.size(); i++){
      std::cout<<"L_DC3  "<<normLDC3[i]<<std::endl;
    }

    normU_BDF2[2 * iter] =
      *std::max_element(normUBDF2.begin(), normUBDF2.end());
    normU_DC3[2 * iter] =
      *std::max_element(normUDC3.begin(), normUDC3.end());  
    normP_BDF2[2 * iter] =
      *std::max_element(normPBDF2.begin(), normPBDF2.end());  
    normP_DC3[2 * iter] =
      *std::max_element(normPDC3.begin(), normPDC3.end());  
    normL_BDF2[2 * iter] =
      *std::max_element(normLBDF2.begin(), normLBDF2.end());
    normL_DC3[2 * iter] =
      *std::max_element(normLDC3.begin() , normLDC3.end()); 
    intL_BDF2[2 * iter] =
      *std::max_element(IntLBDF2.begin(), IntLBDF2.end());    
    intL_DC3[2 * iter] =
      *std::max_element(IntLDC3.begin(), IntLDC3.end()); 
  ...
  for(int i = 1; i < nIter; ++i) {
    normU_BDF2[2 * i + 1] =
      -log(normU_BDF2[2 * i] / normU_BDF2[2 * (i - 1)]) / log(2.);
    normU_DC3[2 * i + 1] =
      -log(normU_DC3[2 * i] / normU_DC3[2 * (i - 1)]) / log(2.);
    normP_BDF2[2 * i + 1] =
      -log(normP_BDF2[2 * i] / normP_BDF2[2 * (i - 1)]) / log(2.);
    normP_DC3[2 * i + 1] =
      -log(normP_DC3[2 * i] / normP_DC3[2 * (i - 1)]) / log(2.); 
    normL_BDF2[2 * i + 1] =
      -log(normL_BDF2[2 * i] / normL_BDF2[2 * (i - 1)]) / log(2.);
    normL_DC3[2 * i + 1] =
      -log(normL_DC3[2 * i] / normL_DC3[2 * (i - 1)]) / log(2.);      
  }
  printf("\n");
  printf("\n");
  printf("%12s \t %12s \t %12s \t %12s \t %12s \t %12s \t \n", "nSteps", "nElm","||u-uh||_BDF2", "tauxU_BDF2", "||u-uh||_DC3", "tauxU_DC3");
  ...
  printf("\n");
  printf("%12s \t %12s \t %12s \t %12s \t %12s \t %12s \n", "nSteps", "nElm",
         "||p-ph||_BDF2", "tauxP_BDF2", "||p-ph||_DC3", "tauxP_DC3");
  for(int i = 0; i < nIter; ++i)
    printf("%12d \t %12d \t %12.6e \t %12.6e \t %12.6e \t %12.6e \n", TT[i], nElm[i],
  ...
  printf("\n");
  printf("%12s \t %12s \t %12s \t %12s \t %12s \t %12s \n", "nSteps", "nElm",
         "||L-Lh||_BDF2", "tauxL_BDF2", "||L-Lh||_DC3", "tauxL_DC3");
  for(int i = 0; i < nIter; ++i)
    printf("%12d \t %12d \t %12.6e \t %12.6e \t %12.6e \t %12.6e \n", TT[i], nElm[i],
