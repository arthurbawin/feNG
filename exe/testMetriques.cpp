...
int main(int argc, char** argv){
  ...
  for(int iter = 0; iter < nIter; ++iter){
    ...
    solveStationary(&normL2_exacte[2*iter], tol, metaNumber, linearSystem, formMatrices, formResiduals, sol, norms, mesh);
  ...
  for(int i = 1; i < nIter; ++i){
    normL2_exacte[2*i+1] = - log(normL2_exacte[2*i]/normL2_exacte[2*(i-1)])/log(sqrt(nNodes[i])/sqrt(nNodes[i-1]));
    normL2_recons[2*i+1] = - log(normL2_recons[2*i]/normL2_recons[2*(i-1)])/log(sqrt(nNodes[i])/sqrt(nNodes[i-1]));
    normL2_recons_exacte[2*i+1] = - log(normL2_recons_exacte[2*i]/normL2_recons_exacte[2*(i-1)])/log(sqrt(nNodes[i])/sqrt(nNodes[i-1]));
  }
  printf("%12s \t %12s \t %12s \t %12s \t %12s \t %12s \t %12s\n", "nNodes", "||u-uh||", "taux", "||p-uh||", "taux", "||u-p||", "taux");
  for(int i = 0; i < nIter; ++i)
  printf("%12d \t %12.6e \t %12.6e \t %12.6e \t %12.6e \t %12.6e \t %12.6e\n", nNodes[i], 
    normL2_exacte[2*i], normL2_exacte[2*i+1],
    normL2_recons[2*i], normL2_recons[2*i+1],
    normL2_recons_exacte[2*i], normL2_recons_exacte[2*i+1]);
