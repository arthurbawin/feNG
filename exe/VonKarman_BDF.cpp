...
// double fSolU_inlet(const double t, const std::vector<double> &pos, const std::vector<double>
// &par) {
//   double y = pos[1];
//   double tEnd = 60.0;
...
// double fSolU_inlet_dot(const double t, const std::vector<double> &pos, const std::vector<double>
// &par) {
//   double y = pos[1];
//   double tEnd = 10.0;
//   double tRamp = fmin(1.0, t / tEnd);
...
int main(int argc, char **argv) {
  ...
  for(int iter = 0; iter < nIter; ++iter) {
    ...
    // std::string meshName = "../../data/VonKarman/TestvonKarman" + std::to_string(iter + 1) +
    // ".msh";

    feMesh2DP1 *mesh = new feMesh2DP1(meshName, false);
    nElm[iter] = mesh->getNbInteriorElems();

    feSpace1DP2 U_walls = feSpace1DP2(mesh, "U", "walls", funZero);
    feSpace1DP2 U_inlet = feSpace1DP2(mesh, "U", "inlet", funSolU_inlet); // Inlet
    feSpace1DP2 U_cylindre = feSpace1DP2(mesh, "U", "cylinder", funZero);
    feSpaceTriP2 U_surface = feSpaceTriP2(mesh, "U", "surface", funZero);

    feSpace1DP2 V_walls = feSpace1DP2(mesh, "V", "walls", funZero);
    feSpace1DP2 V_inlet = feSpace1DP2(mesh, "V", "inlet", funZero); // Inlet
    feSpace1DP2 V_cylindre = feSpace1DP2(mesh, "V", "cylinder", funZero);
    feSpaceTriP2 V_surface = feSpaceTriP2(mesh, "V", "surface", funZero);

    feSpaceTriP1 P_surface = feSpaceTriP1(mesh, "P", "surface", funSolP);

    feSpace1DP2 Lu_cylinder = feSpace1DP2(mesh, "Lu", "cylinder", funZero);
    feSpace1DP2 Lv_cylinder = feSpace1DP2(mesh, "Lv", "cylinder", funZero);

    feSpace1DP2 Bu_cylinder = feSpace1DP2(mesh, "Bu", "cylinder", funZero);
    feSpace1DP2 Bv_cylinder = feSpace1DP2(mesh, "Bv", "cylinder", funZero);

    std::vector<feSpace *> fespace = {
      &U_walls,   &U_inlet,   &U_cylindre,  &U_surface,   &V_walls,     &V_inlet,    &V_cylindre,
    ...
    feBilinearForm *weakBC_UCylinder =
      new feBilinearForm(spaceWeak_U, mesh, nQuad, new feSysElm_1D_weakBC_edo1(1.0, funZero));
    feBilinearForm *weakBC_VCylinder =
      new feBilinearForm(spaceWeak_V, mesh, nQuad, new feSysElm_1D_weakBC_edo1(1.0, funZero));
