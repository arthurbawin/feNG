...
int main(int argc, char **argv) {
  ...
  for(int iter = 0; iter < nIter; ++iter) {
    ...
    std::vector<feSpace *> fespace = {&U_bas,      &U_haut,    &U_gauche, &U_cylindre,
                                      &U_surface,  &V_bas,     &V_haut,   &V_gauche,
                                      &V_cylindre, &V_surface, &P_droite, &P_surface};

    std::vector<feSpace *> feEssBC = {&U_gauche, &U_cylindre, &V_bas,   &V_haut,
                                      &V_gauche, &V_cylindre, &P_droite};
    ...
    // double fooNorm;
    // solveStationary(&fooNorm, tol, metaNumber, linearSystem, formMatrices, formResiduals, sol,
    // norms, mesh);
  ...
  // "||v-vh||", "tauxV", "||p-ph||", "tauxP"); for(int i = 0; i < nIter; ++i) printf("%12d \t
  ...
  //  printf("%12s \t %12s \t %12s\t %12s \t %12s\t %12s \t %12s\n", "nElm", "||E-DC3_U||", "Taux
  //  DC3_U", "||E-DC3_V||", "Taux DC3_V", "||E-DC3_P||", "Taux DC3_P");
  // for(int i = 0; i < nIter; ++i)
  //   printf("%12d \t %12.6e \t %12.6e\t %12.6e \t %12.6e\t %12.6e \t %12.6e\n",
  //     nElm[i], maxNormL2DC3_U[2*i], maxNormL2DC3_U[2*i+1], maxNormL2DC3_V[2*i],
