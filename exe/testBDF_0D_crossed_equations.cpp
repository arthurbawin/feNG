...
// std::vector<double> fSourceU(const double t, const std::vector<double> &x, const std::vector<double> par) {
//   return 7*pow(t,6);
// }
// std::vector<double> fSourceV(const double t, const std::vector<double> &x, const std::vector<double> par) {
//   return 5*pow(t,4);
// }
...
// std::vector<double> fSource(const double t, const std::vector<double> &x, const std::vector<double> par) {
...
int main(int argc, char** argv){
  ...
  for(int iter = 0; iter < nIter; ++iter){
    ...
    feBilinearForm *source_U_M1D = new feBilinearForm(spaceSource1D_U, mesh, nQuad, new feSysElm_0D_Source_crossed(1.0, funSource));
    feBilinearForm *masse_U_M1D = new feBilinearForm(spaceMasse1D_U, mesh, nQuad, new feSysElm_0D_Masse(1.0, nullptr));
  ...
  for(int i = 0; i < nIter; ++i)
    printf("%12d \t %12d \t %12.6e \t %12.6e\n", numberTimeStep[i], nElm[i], normL2[2*i], normL2[2*i+1]);
