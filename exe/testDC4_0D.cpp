...
int main(int argc, char **argv)
{
  ...
  for(int iter = 0; iter < nIter; ++iter) {
    ...
    std::vector<feComputer *> comput = {normBDF2, normDC3, normDC4};
    ...
    double t0 = 0.;
    double t1 = 1.;
    int nTimeSteps = 50 * pow(2, iter);
    TT[iter] = nTimeSteps;
    feTolerances tol{1e-10, 1e-10, 10};
    feCheck(createTimeIntegrator(solver, DC4, tol, system, &metaNumber, &sol, &mesh, comput,
  ...
  printf("%12s \t %12s \t %12s \t %12s \t %12s \t %12s \t %12s \t %12s \n", "nTimeSteps", "nElm",
         "||E_BDF2||", "Taux BDF2", "||E_DC3||", "Taux DC3", "||E_DC4||", "Taux DC4");
  for(int i = 0; i < nIter; ++i)
    printf("%12d \t %12d \t %12.6e \t %12.6e \t %12.6e \t %12.6e \t %12.6e \t %12.6e \n", TT[i],
           nElm[i], normL2_BDF2[2 * i], normL2_BDF2[2 * i + 1], normL2_DC3[2 * i],
           normL2_DC3[2 * i + 1], normL2_DC4[2 * i], normL2_DC4[2 * i + 1]);
