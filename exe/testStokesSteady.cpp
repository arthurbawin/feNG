...
double fSolP(const double t, const std::vector<double> &pos, const std::vector<double> &par){
  double x = pos[0];
  double y = pos[1];
  // return -424. + 156.*exp(1) + (y*y - y) * (-456. + exp(x)*(456. + x*x*(228.-5.*(y*y-y)) + 2.*x*(y*y-y-228.) 
...
void fSource(const double t, const std::vector<double> &pos, const std::vector<double> &par, std::vector<double> &res){
  ...
  // double d2udy2 = 12*exp(x)*x*x*(x-1)*(x-1)*(2*y-1);
  // double d2vdx2 = -exp(x)*y*y*(x*x*x*x + 10*x*x*x + 19*x*x - 6*x - 6)*(y-1)*(y-1);
  ...
  // double dpdx = (y*y - y)*(exp(x)*(4*x*x*x*(y*y - y + 12) + 6*x*x*(y*y - y - 36) + 2*(228 - 5*(y*y - y)) + 2*(y*y - y - 228)) 
  // + exp(x)*(x*x*x*x*(y*y - y + 12) + 2*x*x*x*(y*y - y - 36) + x*x*(228 - 5*(y*y - y)) + 2*x*(y*y - y - 228) + 456));

  // double dpdy = exp(x)*(y*y - y)*(x*x*x*x*(2*y - 1) + 2*x*x*x* (2*y - 1) - 5*x*x*(2*y - 1) + 2*x*(2*y - 1)) 
  // + (2*y - 1)*(exp(x)*(x*x*x*x*(y*y - y + 12) + 2*x*x*x*(y*y - y - 36) + x*x*(228 - 5*(y*y - y)) + 2*x*(y*y - y - 228) + 456) - 456);
