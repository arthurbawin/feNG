...
void feSysElm_1D_Source::computeBe(std::vector<double> &J, int numElem, std::vector<feSpace*> &intSpace, feSpace *geoSpace, 
...
void feSysElm_1D_Diffusion::computeAe(std::vector<double> &J, int numElem, std::vector<feSpace*> &intSpace, feSpace *geoSpace, 
...
void feSysElm_1D_Diffusion::computeBe(std::vector<double> &J, int numElem, std::vector<feSpace*> &intSpace, feSpace *geoSpace, 
  std::vector<double> &geoCoord, double c0, double tn, double* Be){
  int                nG = geoSpace->getNbQuadPoints();
  std::vector<double> w = geoSpace->getQuadratureWeights();
  double             kD = _par;
...
void feSysElm_1D_Masse::computeAe(std::vector<double> &J, int numElem, std::vector<feSpace*> &intSpace, feSpace *geoSpace, 
  std::vector<double> &geoCoord, double c0, double tn, double** Ae){
  int                nG = geoSpace->getNbQuadPoints();
  std::vector<double> w = geoSpace->getQuadratureWeights();
  double            rho = _par;
...
void feSysElm_1D_Masse::computeBe(std::vector<double> &J, int numElem, std::vector<feSpace*> &intSpace, feSpace *geoSpace, 
  std::vector<double> &geoCoord, double c0, double tn, double* Be){
  int                nG = geoSpace->getNbQuadPoints();
  std::vector<double> w = geoSpace->getQuadratureWeights();
  double            rho = _par;
...
void feSysElm_2D_Source::computeBe(std::vector<double> &J, int numElem, std::vector<feSpace*> &intSpace, feSpace *geoSpace, 
  std::vector<double> &geoCoord, double c0, double tn, double* Be){
  int                nG = geoSpace->getNbQuadPoints();
  std::vector<double> w = geoSpace->getQuadratureWeights();
  int        nFunctions = intSpace[_idU]->getNbFunctions();
...
void feSysElm_2D_Diffusion::computeAe(std::vector<double> &Ja, int numElem, std::vector<feSpace*> &intSpace, feSpace *geoSpace, 
  std::vector<double> &geoCoord, double c0, double tn, double** Ae){
  int                nG = geoSpace->getNbQuadPoints();
  std::vector<double> w = geoSpace->getQuadratureWeights();
  double             kD = _par;
...
void feSysElm_2D_Diffusion::computeBe(std::vector<double> &Ja, int numElem, std::vector<feSpace*> &intSpace, feSpace *geoSpace, 
  std::vector<double> &geoCoord, double c0, double tn, double* Be)
{
  int                nG = geoSpace->getNbQuadPoints();
  std::vector<double> w = geoSpace->getQuadratureWeights();
...
void feSysElm_2D_Stokes::computeBe(std::vector<double> &Ja, int numElem, std::vector<feSpace*> &intSpace, feSpace *geoSpace, 
  std::vector<double> &geoCoord, double c0, double tn, double* Be)
{
  ...
  for(int k = 0; k < nG; ++k){
    ...
    for(int i = 0; i < nFunctionsU; ++i){
      _feU[i]   = intSpace[_idU]->getFunctionAtQuadNode(i,k); // Fonction test de u : uniquement pour les forces volumiques
      _feUdx[i] = intSpace[_idU]->getdFunctiondrAtQuadNode(i,k) * drdx + 
                  intSpace[_idU]->getdFunctiondsAtQuadNode(i,k) * dsdx;
      _feUdy[i] = intSpace[_idU]->getdFunctiondrAtQuadNode(i,k) * drdy +
                  intSpace[_idU]->getdFunctiondsAtQuadNode(i,k) * dsdy;
    ...
    for(int i = 0; i < nFunctionsV; ++i){
      _feV[i]   = intSpace[_idV]->getFunctionAtQuadNode(i,k); // Fonction test de v : uniquement pour les forces volumiques
...
void feSysElm_2D_Stokes::computeAe(std::vector<double> &Ja, int numElem, std::vector<feSpace*> &intSpace, feSpace *geoSpace, 
  std::vector<double> &geoCoord, double c0, double tn, double** Ae)
{
  // TODO : Verifier que les règles d'intégration soient identiques ou adapter
  // On prend les poids de geoSpace, ok pour le jacobien seulement ?
  ...
  for(int k = 0; k < nG; ++k){
    ...
    for(int i = 0; i < nFunctionsU; ++i){
      _feU[i]   = intSpace[_idU]->getFunctionAtQuadNode(i,k);
      _feUdx[i] = intSpace[_idU]->getdFunctiondrAtQuadNode(i,k) * drdx + intSpace[_idU]->getdFunctiondsAtQuadNode(i,k) * dsdx;
      _feUdy[i] = intSpace[_idU]->getdFunctiondrAtQuadNode(i,k) * drdy + intSpace[_idU]->getdFunctiondsAtQuadNode(i,k) * dsdy;
    }
    for(int i = 0; i < nFunctionsV; ++i){
      _feV[i]   = intSpace[_idV]->getFunctionAtQuadNode(i,k);
      _feVdx[i] = intSpace[_idV]->getdFunctiondrAtQuadNode(i,k) * drdx + intSpace[_idU]->getdFunctiondsAtQuadNode(i,k) * dsdx;
      _feVdy[i] = intSpace[_idV]->getdFunctiondrAtQuadNode(i,k) * drdy + intSpace[_idU]->getdFunctiondsAtQuadNode(i,k) * dsdy;
    }
    for(int i = 0; i < nFunctionsP; ++i){
      _feP[i]   = intSpace[_idP]->getFunctionAtQuadNode(i,k);
    }
    ...
    for(int i = 0; i < nFunctionsU; ++i){
      J = 0;
      for(int j = 0; j < nFunctionsU; ++j){ Ae[I][J++] += jac * w[k] * (_feUdx[i] * mu * 2. * _feUdx[j] + _feUdy[i] * mu * _feUdy[j]); }
      for(int j = 0; j < nFunctionsV; ++j){ Ae[I][J++] += jac * w[k] * (_feUdy[i] * mu * _feVdx[j]);                                   }
    ...
    for(int i = 0; i < nFunctionsV; ++i){
      J = 0;
      for(int j = 0; j < nFunctionsU; ++j){ Ae[I][J++] += jac * w[k] * (_feVdx[i] * mu * _feUdy[j]);                                   }
      for(int j = 0; j < nFunctionsV; ++j){ Ae[I][J++] += jac * w[k] * (_feVdy[i] * mu * 2. * _feVdy[j] + _feVdx[i] * mu * _feVdx[j]); }
...
void feSysElm_2D_NavierStokes::computeBe(std::vector<double> &Ja, int numElem, std::vector<feSpace*> &intSpace, feSpace *geoSpace, 
  std::vector<double> &geoCoord, double c0, double tn, double* Be)
{
  // TODO : Verifier que les règles d'intégration soient identiques ou adapter
  // On prend les poids de geoSpace, ok pour le jacobien seulement ?
  ...
  for(int k = 0; k < nG; ++k){
    ...
    for(int i = 0; i < nFunctionsU; ++i){
      ...
      _feUdy[i] = intSpace[_idU]->getdFunctiondrAtQuadNode(i,k) * drdy +
                  intSpace[_idU]->getdFunctiondsAtQuadNode(i,k) * dsdy;
      Be[cnt++] -= (_feU[i] * rho * (dudt + u * dudx + v * dudy) + Sxx * _feUdx[i] + Sxy * _feUdy[i] - f[0] * _feU[i]) * J * w[k];
    }
    // Residu pour v
    for(int i = 0; i < nFunctionsV; ++i){
      _feV[i]   = intSpace[_idV]->getFunctionAtQuadNode(i,k);
      ...
      _feVdy[i] = intSpace[_idV]->getdFunctiondrAtQuadNode(i,k) * drdy +
                  intSpace[_idV]->getdFunctiondsAtQuadNode(i,k) * dsdy;
      Be[cnt++] -= (_feV[i] * rho * (dvdt + u * dvdx + v * dvdy) + Syx * _feVdx[i] + Syy * _feVdy[i] - f[1] * _feV[i]) * J * w[k];
    }
    // Residu pour p
    for(int i = 0; i < nFunctionsP; ++i){
      _feP[i] = intSpace[_idP]->getFunctionAtQuadNode(i,k);
...
void feSysElm_2D_NavierStokes::computeAe(std::vector<double> &Ja, int numElem, std::vector<feSpace*> &intSpace, feSpace *geoSpace, 
  std::vector<double> &geoCoord, double c0, double tn, double** Ae)
{
  // TODO : Verifier que les règles d'intégration soient identiques ou adapter
  // On prend les poids de geoSpace, ok pour le jacobien seulement ?
  ...
  for(int k = 0; k < nG; ++k){
    ...
    for(int i = 0; i < nFunctionsU; ++i){
      J = 0;
      for(int j = 0; j < nFunctionsU; ++j){ Ae[I][J++] += jac * w[k] * (rho*_feU[i]*(c0*_feU[j] + u*_feUdx[j] + v*_feUdy[j] + _feU[j]*dudx) + _feUdx[i] * mu * 2. * _feUdx[j] + _feUdy[i] * mu * _feUdy[j]); }
      for(int j = 0; j < nFunctionsV; ++j){ Ae[I][J++] += jac * w[k] * (rho*_feU[i]*(                            _feV[j]*dudy) + _feUdy[i] * mu * _feVdx[j]);                                   }
      for(int j = 0; j < nFunctionsP; ++j){ Ae[I][J++] -= jac * w[k] * (_feUdx[i] * _feP[j]);                                          }
      ++I;
    }
    ...
    for(int i = 0; i < nFunctionsV; ++i){
      J = 0;
      for(int j = 0; j < nFunctionsU; ++j){ Ae[I][J++] += jac * w[k] * (rho*_feV[i]*(                            _feU[j]*dvdx) + _feVdx[i] * mu * _feUdy[j]);                                   }
      for(int j = 0; j < nFunctionsV; ++j){ Ae[I][J++] += jac * w[k] * (rho*_feV[i]*(c0*_feV[j] + u*_feVdx[j] + v*_feVdy[j] + _feV[j]*dvdy) + _feVdy[i] * mu * 2. * _feVdy[j] + _feVdx[i] * mu * _feVdx[j]); }
      for(int j = 0; j < nFunctionsP; ++j){ Ae[I][J++] -= jac * w[k] * (_feVdy[i] * _feP[j]);                                          }
      ++I;
    }
