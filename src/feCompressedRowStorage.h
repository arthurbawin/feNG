...
{
public:
	feCompressedRowStorage(feMetaNumber *metaNumber, feMesh *mesh, std::vector<feBilinearForm*> &formMatrices);
...
{
public:
	feCompressedRowStorageMklPardiso(feMetaNumber *metaNumber, feMesh *mesh, std::vector<feBilinearForm*> &formMatrices);
	...
	void    freeMatrix(double* Matrix) {if(Matrix != NULL) delete [] Matrix;};
	void    zeroMatrix(double* Matrix) {for(feInt i=0;i<nz;i++) Matrix[i] = 0;};
    // ================================================================
    // Somme des coefficients dans la matrice 
    // ================================================================	
    void  matrixAddValues(double* Matrix, feInt nRow, feInt *Row, feInt nColumn, feInt *Column, double** Aij);
