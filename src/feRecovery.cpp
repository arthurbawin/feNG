...
static bool isBoundary(feMesh *mesh, int vertex){
  // return false;
  return fabs(mesh->getVertex(vertex)->x()) < 1e-10 || fabs(mesh->getVertex(vertex)->x() - 5.0) < 1e-10;
}

static bool isBoundary(feMesh *mesh, Edge edge){
  return false;
}

static double boundaryCondition(feMesh *mesh, int vertex, feFunction *solRef){
  return solRef->eval(0, {mesh->getVertex(vertex)->x(), mesh->getVertex(vertex)->y(), mesh->getVertex(vertex)->z()} );
}

static void boundaryConditionVec(feMesh *mesh, int vertex, feVectorFunction *solRefGrad, std::vector<double> &res){
  solRefGrad->eval(0, {mesh->getVertex(vertex)->x(), mesh->getVertex(vertex)->y(), mesh->getVertex(vertex)->z()}, res);
}

static double boundaryCondition(feMesh *mesh, Edge edge, feFunction *solRef){
  return 0.0;
}

static void boundaryConditionVec(feMesh *mesh, Edge edge, feVectorFunction *solRefGrad, std::vector<double> &res){
}
  ...
  switch(mesh->getDim()){
    case 1 :
      ...
      for(auto e : mesh->_edges){
        edgeToElems[e.getTag()].insert(vertToElems[e.getTag(0)].begin(), vertToElems[e.getTag(0)].end());
        edgeToElems[e.getTag()].insert(vertToElems[e.getTag(1)].begin(), vertToElems[e.getTag(1)].end());
      }
      ...
      // vertToElems[0].clear();
      // vertToElems[0].insert(0);
      // // vertToElems[0].insert(1);
      // // vertToElems[0].insert(2);
      // // vertToElems[0].insert(3);
      // // vertToElems[0].insert(4);
      // // vertToElems[0].insert(5);
      // // vertToElems[0].insert(6);
      // vertToElems[40].clear();
      // vertToElems[40].insert(39);
      // vertToElems[40].insert(38);
      // vertToElems[40].insert(37);
      // vertToElems[40].insert(36);
      break;
    case 2 :
...
feRecovery::feRecovery(feMetaNumber *metaNumber, feSpace *space, feMesh *mesh, feSolution *sol,
                       ...
                       std::string metricMeshName, feVectorFunction *solRefGrad, feVectorFunction *solRefHess, feFunction *fund3udx, bool append)
  : _metaNumber(metaNumber), _mesh(mesh), _sol(sol), _intSpace(space), _solRef(solRef), _solRefGrad(solRefGrad), _solRefHess(solRefHess){
  ...
  switch(_dim){
    case 1 : matrixInverseEigen1D(); break;
    case 2 : matrixInverseEigen2D(); break;
  }
  printf("Done\n");
  ...
  // for(int iDerivative = 0; iDerivative < 1; ++iDerivative) {
    ...
    // if(iDerivative < 2){
      ...
      switch(_dim){
        case 1 : solveLeastSquareEigen1D(pow(_dim, iDerivative), iDerivative); break;
        case 2 : solveLeastSquareEigen2D(pow(_dim, iDerivative), iDerivative); break;
      }
    // }
    ...
    if(iDerivative == 2 && fund3udx != nullptr) { estimated3Error(norm, fund3udx); }
  }
...
void feRecovery::matrixInverseEigen1D(){
  ...
  std::vector<double> geoCoord, x(3, 0.0), monomials(_dimRecovery, 0.);
  ...
  for(auto v : _patch->getVertices()) {
      ...
      for(int k = 0; k < nQuad; ++k) {
        ...
        for(int i = 0; i < _dimRecovery; ++i) {
          for(int j = 0; j < _dimRecovery; ++j) {
            switch(_degSol){
              case 1: myMat3(i,j) += jac * w[k] * monomials[i] * monomials[j]; break;
              case 2: 
    ...
    switch(_degSol){
      case 1: lsInvAtVertices3[v] = myMat3.inverse(); break;
      case 2:
  ...
  for(auto e : _mesh->_edges) {
    ...
    for(auto elem : elemPatch) {
      ...
      for(int k = 0; k < nQuad; ++k) {
        double jac = J[nQuad * elem + k];
        _geoSpace->interpolateVectorFieldAtQuadNode(geoCoord, k, x);
        double xLoc = x[0] - (_mesh->getVertex(e.getTag(0))->x() + _mesh->getVertex(e.getTag(1))->x())/2.0;
        // printf("Edge %d (%2.2f)- Elem %d - Quad node %d (%2.2f) - xLoc = %2.2f\n",
        //   e.getTag(), (_mesh->getVertex(e.getTag(0))->x() + _mesh->getVertex(e.getTag(1))->x())/2.0, elem, k, x[0], xLoc);
        for(int i = 0; i < _dimRecovery; ++i)
          monomials[i] = pow(xLoc, _expXRecovery[i]);
    ...
    switch(_degSol){
      case 1: lsInvAtVertices3OnEdges[e.getTag()] = myMat3.inverse(); break;
      case 2:
      ...
      for(int k = 0; k < nQuad; ++k) {
        ...
        if(iDerivative > 0) {
          ...
          for(int iDeriv = 0; iDeriv < indRecovery; ++iDeriv){
            ...
            u[iDeriv] += _intSpace->getFunctionAtQuadNode(2, k) * du[0]; // FIXME : Évaluer proprement la fonction d'interpolation
          }
        ...
        for(int iDeriv = 0; iDeriv < indRecovery; ++iDeriv){
          for(int i = 0; i < _dimRecovery; ++i){
            switch(_degSol){
              case 1: RHS3[iDeriv](i) += jac * w[k] * u[iDeriv] * monomials[i]; break; 
              case 2:
    ...
    for(int iDeriv = 0; iDeriv < indRecovery; ++iDeriv){
      switch(_degSol){
        ...
        case 2:
        {
          ...
          std::cout<<std::endl;
          // recoveryCoeff[v][iDeriv] = std::vector<double>(sol.data(), sol.data() + sol.rows() * sol.cols());
          std::vector<double> solContVec = std::vector<double>(solCont.data(), solCont.data() + solCont.rows() * solCont.cols());
          solContVec.pop_back();
          if(iDerivative < 3 && isBoundary(_mesh,v)){
...
void feRecovery::derivative(int indRecovery, int iDerivative, std::ostream &output) {
  ...
  if(iDerivative == 0){
    ...
    for(auto v : vertices)
      solV[v] = recoveryCoeff[v][0][0];
    derivAtVertices.push_back(solV);
    std::vector<double> solE(_mesh->getNbEdges(),0.0);
    for(auto e : _mesh->_edges)
      solE[e.getTag()-1] = recoveryCoeffOnEdges[e.getTag()][0][0][0];
  ...
  if(_dim == 1) {
    ...
    for(auto v : vertices) {
      ...
      for(int i = 0; i < _dimRecovery; ++i){
        if(_expXRecovery[i] != 0) { sumCoeffX += ((double) _expXRecovery[i]) * fabs(u[i]); }
        if(_expYRecovery[i] != 0) { sumCoeffY += ((double) _expYRecovery[i]) * fabs(u[i]); }
      }
      for(int i = 0; i < _dimRecovery; ++i) {
    ...
    for(auto e : _mesh->_edges) {
      ...
      // for(int i = 0; i < _dimRecovery; ++i) {
      //   if(_expXRecovery[i] != 0 && fabs(u[i]/sumCoeffX) > tol) { dudx[indX++] = _expXRecovery[i] * u[i]; }
      //   if(_expYRecovery[i] != 0 && fabs(u[i]/sumCoeffY) > tol) { dudy[indY++] = _expYRecovery[i] * u[i]; }
      // }
      for(int i = 0; i < _dimRecovery; ++i) {
        if(_expXRecovery[i] != 0){
          if(fabs(u[i]/sumCoeffX) > tol){
  ...
  // for(auto iElm : elemPatch) {
    ...
    for(int k = 0; k < nQuad; ++k) {
      ...
      for(int iEdge = 0; iEdge < _nEdgePerElm; ++iEdge){
        ...
        if(_dim == 1) uReconstruit += _intSpace->getFunctionAtQuadNode(iEdge+2, k) * recoveryCoeffOnEdges[localEdge][0][0][0];
        if(_dim == 2) uReconstruit += _intSpace->getFunctionAtQuadNode(iEdge+3, k) * recoveryCoeffOnEdges[localEdge][0][0][0]; // FIXME : make this clean (iEdge+2)
      }
      ...
      //     // u[iDeriv] += _geoSpace->getFunctionAtQuadNode(iVert, k) * du[i] * pow(xLoc[0], _expX[i]) * pow(xLoc[1], _expY[i]);
      //     // u[iDeriv] += _intSpace->getFunctionAtQuadNode(iVert, k) * du[i] * pow(xLoc[0], _expX[i]) * pow(xLoc[1], _expY[i]);
      //     // uReconstruit += _intSpace->getFunctionAtQuadNode(iVert, k) * recoveryCoeff[vNode][0][i] * pow(xLoc[0], _expX[i]);
      //     uReconstruit += _geoSpace->getFunctionAtQuadNode(iVert, k) * recoveryCoeff[vNode][0][i] * pow(xLoc[0], _expX[i]);
      //   }
      // }
      ...
      fprintf(f, "%+-12.12e \t %+-12.12e \t %+-12.12e \t %+-12.12e \t %+-12.12e\n", x[0], x[1], uReconstruit, solRef->eval(0, x), _intSpace->interpolateSolutionAtQuadNode(k));
    }
  }
  norm[0] = sqrt(norm[0]);
  norm[1] = sqrt(norm[1]);
    ...
    for(int k = 0; k < nQuad; ++k) {
      ...
      for(int iVert = 0; iVert < _nVertPerElm; ++iVert){ 
        ...
        // for(int i = 0; i < _dimDerivation; ++i) {
          ...
          if(_dim == 2) dudyReconstruit += _intSpace->getFunctionAtQuadNode(iVert, k) * recoveryCoeff[vNode][1][0];
        // }
      }
      // Edges
      for(int iEdge = 0; iEdge < _nEdgePerElm; ++iEdge){
        ...
        // for(int i = 0; i < _dimDerivation; ++i) {
          ...
          if(_dim == 1) dudxReconstruit += _intSpace->getFunctionAtQuadNode(iEdge+2, k) * recoveryCoeffOnEdges[localEdge][0][0][0];

          if(_dim == 2) dudxReconstruit += _intSpace->getFunctionAtQuadNode(iEdge+3, k) * recoveryCoeffOnEdges[localEdge][0][0][0];
          if(_dim == 2) dudyReconstruit += _intSpace->getFunctionAtQuadNode(iEdge+3, k) * recoveryCoeffOnEdges[localEdge][0][1][0];
        // }
      }
    ...
    for(int k = 0; k < nQuad; ++k) {
      ...
      for(int iVert = 0; iVert < _nVertPerElm; ++iVert){ 
        ...
        // for(int i = 0; i < _dimDerivation; ++i) {
          ...
          if(_dim == 2) dudyReconstruit += _intSpace->getFunctionAtQuadNode(iVert, k) * derivativeCoeff[vNode][1][0];
          // u[iDeriv] += _intSpace->getFunctionAtQuadNode(iVert, k) * du[i] * pow(xLoc[0], _expX[i]) * pow(xLoc[1], _expY[i]);
        // }
      }
      // Edges
      for(int iEdge = 0; iEdge < _nEdgePerElm; ++iEdge){
        ...
        // for(int i = 0; i < _dimDerivation; ++i) {
          ...
          if(_dim == 1) dudxReconstruit += _intSpace->getFunctionAtQuadNode(iEdge+2, k) * derivativeCoeffOnEdges[localEdge][0][0][0];

          if(_dim == 2) dudxReconstruit += _intSpace->getFunctionAtQuadNode(iEdge+3, k) * derivativeCoeffOnEdges[localEdge][0][0][0];
          // dudyReconstruit += _intSpace->getFunctionAtQuadNode(iEdge+3, k) * derivativeCoeffOnEdges[localEdge][0][1][i] * monomials[i];
          if(_dim == 2) dudyReconstruit += _intSpace->getFunctionAtQuadNode(iEdge+3, k) * derivativeCoeffOnEdges[localEdge][0][1][0];
        // }
      }
  ...
  // for(auto iElm : elemPatch) {
    ...
    for(int k = 0; k < nQuad; ++k) {
      ...
      for(int iVert = 0; iVert < _nVertPerElm; ++iVert){ 
        ...
        if(_dim == 2) d2udxyReconstruit += _intSpace->getFunctionAtQuadNode(iVert, k) * derivativeCoeff[vNode][1][0];
        if(_dim == 2) d2udyxReconstruit += _intSpace->getFunctionAtQuadNode(iVert, k) * derivativeCoeff[vNode][2][0];
        if(_dim == 2) d2udy2Reconstruit += _intSpace->getFunctionAtQuadNode(iVert, k) * derivativeCoeff[vNode][3][0];
      }
      // Edges
      for(int iEdge = 0; iEdge < _nEdgePerElm; ++iEdge){
        int localEdge = fabs(_cnc->getEdgeConnectivity(iElm, iEdge));
        ...
        // }
        if(_dim == 1) d2udx2Reconstruit += _intSpace->getFunctionAtQuadNode(iEdge+2, k) * derivativeCoeffOnEdges[localEdge][0][0][0];

        if(_dim == 2) d2udx2Reconstruit += _intSpace->getFunctionAtQuadNode(iEdge+3, k) * derivativeCoeffOnEdges[localEdge][0][0][0];
        if(_dim == 2) d2udxyReconstruit += _intSpace->getFunctionAtQuadNode(iEdge+3, k) * derivativeCoeffOnEdges[localEdge][0][1][0];
        if(_dim == 2) d2udyxReconstruit += _intSpace->getFunctionAtQuadNode(iEdge+3, k) * derivativeCoeffOnEdges[localEdge][0][2][0];
        if(_dim == 2) d2udy2Reconstruit += _intSpace->getFunctionAtQuadNode(iEdge+3, k) * derivativeCoeffOnEdges[localEdge][0][3][0];
      }
      ...
      if(solRefHess) {
        ...
        fprintf(f, "%+-12.12e \t %+-12.12e \t %+-12.12e \t %+-12.12e\n", x[0], x[1], d2udx2Reconstruit, res[0]);
      }
  ...
  // for(auto v : _patch->getVertices()){
    ...
    // for(auto iElm : elemPatch) {
      ...
      for(int k = 0; k < nQuad; ++k) {
        ...
        for(int iVert = 0; iVert < _nVertPerElm; ++iVert){ 
          int vNode = _cnc->getNodeConnectivity(iElm, iVert);
          d3udx3Reconstruit += _intSpace->getFunctionAtQuadNode(iVert, k) * derivativeCoeff[vNode][0][0];
        }
        // Edges
        for(int iEdge = 0; iEdge < _nEdgePerElm; ++iEdge){
          int localEdge = fabs(_cnc->getEdgeConnectivity(iElm, iEdge));
          if(_dim == 1) d3udx3Reconstruit += _intSpace->getFunctionAtQuadNode(iEdge+2, k) * derivativeCoeffOnEdges[localEdge][0][0][0];
        }
        ...
        fprintf(f, "%+-12.12e \t %+-12.12e \t %+-12.12e \t %+-12.12e\n", x[0], x[1], d3udx3Reconstruit, fund3udx->eval(0, x));
      }
    }
...
double feRecovery::evalDerivative(int indexDerivative, std::vector<double> &x){
  ...
  if(!isFound){
    printf("In feRecovery::evalDerivative : Warning - Point (%f, %f, %f) was not found in the mesh.\n", x[0], x[1], x[2]);
    return 0.0;
  } else{
    std::vector<double> nodalValues(6,0.0);
    nodalValues[0] = derivAtVertices[indexDerivative][_cnc->getNodeConnectivity(elm, 0)];
    nodalValues[1] = derivAtVertices[indexDerivative][_cnc->getNodeConnectivity(elm, 1)];
    nodalValues[2] = derivAtVertices[indexDerivative][_cnc->getNodeConnectivity(elm, 2)];
    nodalValues[3] = derivAtEdges[indexDerivative][fabs(_cnc->getEdgeConnectivity(elm, 0))-1];
    nodalValues[4] = derivAtEdges[indexDerivative][fabs(_cnc->getEdgeConnectivity(elm, 1))-1];
    nodalValues[5] = derivAtEdges[indexDerivative][fabs(_cnc->getEdgeConnectivity(elm, 2))-1];
    return _intSpace->interpolateField(nodalValues,u.data());
  }
