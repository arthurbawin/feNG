...
int feMesh2DP1::readMsh2(std::istream &input, bool curved, mapType physicalEntitiesDescription){
  ...
  while(input >> buffer){
    ...
    if(buffer == "$PhysicalNames"){ // Read physical entities
      ...
      // Save the description of the physical entities : this is used when looping and adapting the mesh.
      // Since mmg does not keep track of the name of the physical entities, they have to be saved and loaded
      // during the following adaptation cycles.
      for(int i = 0; i < _nPhysicalEntities; ++i){
    ...
    // There is no Entities block in msh2 : entities information is in the elements
      ...
      for(int iElm = 0; iElm < _nElm; ++iElm){
        ...
        // Since we only accept non-overlapping physical domains (i.e. each element belongs to a single physical entity),
        // let's just assume the first number is the physical domain and the following are the geometric entities.
    ...
    if(buffer == "$PhysicalNames"){ // Read physical entities
      ...
      for(int i = 0; i < _nPoints; ++i){
        ...
        if(e.numPhysicalTags > 1){
          printf("In readGmsh : Error - Geometric entity can only be part of a maximum of one physical entity (named domain).\n");
...
int feMesh2DP1::readGmsh(std::string meshName, bool curved, mapType physicalEntitiesDescription){
      ...
      if(e.dim > 1){ // Not considering b-rep for curves at the moment
        if(e.numPhysicalTags > 0){ // if e is part of a physical entity
          ...
          for(int boundedByE : e.isBoundedBy){ // loop over geometric entities bounding e...
            ...
            // Basically unique() :
            if( std::find(_physicalEntities[tag].isBoundedBy.begin(), _physicalEntities[tag].isBoundedBy.end(), boundedByTag) == _physicalEntities[tag].isBoundedBy.end() )
              _physicalEntities[tag].isBoundedBy.push_back(boundedByTag);
            if( std::find(_physicalEntities[boundedByTag].isBoundaryOf.begin(), _physicalEntities[boundedByTag].isBoundaryOf.end(), tag) == _physicalEntities[boundedByTag].isBoundaryOf.end() )
  ...
  // Use the b-rep to transfer edge/face connectivity to boundary elements (actually, not even using the b-rep...)
  int maxDim = 0;
  for(auto& pE : _physicalEntities){
      ...
      if(pE.dim == maxDim-1){
        ...
        for(int i = 0; i < pE.nElm; ++i){
          ...
          if(it != _edges.end()){
            ...
            // std::cout<<"edge "<< pE.connecEdges[i] << " : (" << v0->getTag() << " - "<<v1->getTag()<<") added in connecEdge of "<<pE.name<<std::endl;
            ...
            printf("In readMesh : Error - Boundary edge (%d,%d) was not found in the set of edges...\n", v0->getTag(), v1->getTag());
            return 1;
          }
        }
      }
  ...
  for(auto& pE : _physicalEntities){
    ...
    else if(pE.cncID == "TriP2"  ){ pE.geoSpace = new feSpaceTriP2("xyz"); }
     else{
