...
public:
  ...
  virtual feStatus
  restartFromContainer(const feSolutionContainer &container) = 0;

  std::vector<std::vector<double>> getPostProcessingData()
  {
...
protected:
  ...
  feStatus postProcessingAndVisualization(const int           localStep,
                                          const int           globalStep,
                                          const double        t,
                                          const feExportData &data);
};
...
public:
  StationaryIntegrator(feLinearSystem        *linearSystem,
                       ...
                       std::vector<feNorm *> &postProcessing,
                       feExportData          &exportData);
  ~StationaryIntegrator() { delete _sC; };
...
public:
  BDF2Integrator(feLinearSystem        *linearSystem,
                 ...
                 double                 tEnd,
                 int                    nTimeSteps);
  ~BDF2Integrator() { delete _sC; };

  void setStartingMethod(const BDF2Starter starter) { _startMethod = starter; }
