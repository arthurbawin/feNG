...
void glfemDrawCurveDG(double *x, double *y, int n, int m)
{
  ...
  glPointSize(8.0);
  glBegin(GL_POINTS);
  for(i = 0; i < n; i++) glVertex2d(x[i * m], y[i * m]);
  for(i = 1; i < n + 1; i++) glVertex2d(x[i * m - 1], y[i * m - 1]);
  glEnd();
  glBegin(GL_LINES);
  for(i = 0; i < n; i++)
    for(j = 0; j < m - 1; j++) {
...
void glfemReshapeWindowsBox(double minX, double maxX, double minY, double maxY, int w, int h)
{
  ...
  double left = meanX - size * fmax(1.0, 1.0 / ratio);
  double right = meanX + size * fmax(1.0, 1.0 / ratio);
  double bottom = meanY - size * fmax(1.0, ratio);
  double top = meanY + size * fmax(1.0, ratio);
