                                     ...
                                     const std::vector<feSpace *> &feSpaces)
{
  switch(format) {
    case VTK:
      ...
      break;
    case TXT:
      exporter = new feExporterTXT(mesh, solution, metaNumber, feSpaces);
      break;
    default:
...
void feExporterTXT::writeHeader(std::ostream &output)
{ 
  std::vector<feSpace *> spacesToExport;
  for(feSpace *fS : _spaces) {
    if(fS->getDim() == _mesh->getDim()) {
  ...
  output << "t  ";
  for(feSpace *fS : spacesToExport){
    output << fS->getFieldID()<<"  ";  
  }
  output<<"\n";
}


void feExporterTXT::writeField(std::ostream &output, feCncGeo *cnc, feSpace *intSpace,
                               std::string fieldID, bool LoopOverCnc)
{
  std::vector<double> &sol = _sol->getSolutionReference();
  feNumber *n = _metaNumber->getNumbering(fieldID);
  int iDOF = n->getDOFNumberAtVertex(0);
  output <<sol[iDOF] <<"  ";
  // printf("%24.18e " , sol[iDOF] );
...
feStatus feExporterTXT::writeStep(std::string fileName)
{
  std::filebuf fb;
  feCncGeo *cnc;
  std::vector<feSpace *> spacesToExport;
  ...
  if(fb.open(fileName, std::ios::out | std::ios::app)) {
    std::ostream output(&fb);
    if(_HeaderStatusBDF1 == 0 && fileName=="../../data/StiffResults/RoberstonBDF1.txt" ){ //TO CHANGE
      writeHeader(output);
      _HeaderStatusBDF1 = 1;
    }
     else if(_HeaderStatusBDF2 == 0 && fileName=="../../data/StiffResults/RoberstonBDF2.txt" ){ //TO CHANGE
      writeHeader(output);
      _HeaderStatusBDF2 = 1;
    }
     else if(_HeaderStatusDC3 == 0 && fileName=="../../data/StiffResults/RoberstonDC3.txt" ){//TO CHANGE
      writeHeader(output);
      _HeaderStatusDC3 = 1;
    }
    else if(_HeaderStatusDC4 == 0 && fileName=="../../data/StiffResults/RoberstonDC4.txt" ){//TO CHANGE
      writeHeader(output);
      _HeaderStatusDC4 = 1;
    }
    double t = _sol->getCurrentTime();
    output <<t<<" ";
    for(feSpace *fS : spacesToExport) {
      writeField(output, cnc, fS, fS->getFieldID(), false);
    }
    ...
    return feErrorMsg(FE_STATUS_FILE_CANNOT_BE_OPENED, "Could not open output files.");
  }
