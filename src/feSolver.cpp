                ...
                feLinearSystem *linearSystem, feSolution *sol, feMesh *mesh)
{
  ...
  while(newton) {
    ...
    feInfoCond(FE_VERBOSE > 0, "    iter %2d : ||A*dx-res|| = %10.10e (%4d iter.) \t ||dx|| = %10.10e \t ||res|| = %10.10e", ++iter, normAxb, linearSystemIter, normDx, normResidual);
    
    newton = !((normDx <= tol.tolDx && normResidual <= tol.tolResidual) || iter > tol.maxIter);

    if((iter > 2 || prev_status == 0) && ((normResidual / _normR0) < 0.001) && cnt < 5) {
      linearSystem->setRecomputeStatus(false);
    } else if(iter == 1 && prev_status == 0){
        if(normResidual < 0.1){
          linearSystem->setRecomputeStatus(false); // we decide to try with the old matrix at the first // iteration just to see if it's enough
        }else{
...
feStatus BDF2Solver::makeSteps(int nSteps)
{
  ...
  if(_currentStep == 0) {
    ...
    if(_CodeIni == "BDF1/DCF") {
      ...
      DC2FSolver solver(_tol, _metaNumber, _linearSystem, _sol, _comput, _mesh, _exportData, _t0, _t_ini, 1);
      solver.makeSteps(1);
...
feStatus DC2FSolver::makeSteps(int nSteps)
{
  ...
  for(int i = 0; i < nSteps; ++i) {
    ...
    initializeDC2F(_sol, _metaNumber, _mesh, dynamic_cast<feSolutionBDF1 *>(_solutionContainerBDF1),dynamic_cast<feSolutionDC2F *>(_solutionContainer));
    printf("\n");
                   ...
                   nTimeSteps, CodeIni)
{
  ...
  _solutionContainerBDF2 = new feSolutionBDF2(nSol, _sol->getCurrentTime(), _metaNumber);
  _solutionContainerDC3F = new feSolutionDCF(Nb_pas_de_temps + 1, _sol->getCurrentTime(), _metaNumber);
  _solutionContainerBDF2->initialize(_sol, _mesh, _metaNumber);
