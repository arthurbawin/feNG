...
DC3FSolver_centered::DC3FSolver_centered(feTolerances tol, feMetaNumber *metaNumber,
                                         feLinearSystem *linearSystem, feSolution *sol,
                                         std::vector<feNorm *> &norms, feMesh *mesh, feExportData exportData, double t0,
                                         double tEnd, int nTimeSteps)
  : TimeIntegrator(tol, metaNumber, linearSystem, sol, norms, mesh, exportData, t0, tEnd, nTimeSteps) {
  // Initialize the solution container
  int nSol = 5;
...
feStatus DC3Solver::makeSteps(int nSteps) {
  ...
  if(_currentStep == 0) {
    if(_CodeIni == "BDF1/DCF") {
      ...
      DC3FSolver solver(_tol, _metaNumber, _linearSystem, _sol, norms2, _mesh, _exportData, _t0, _t_ini,
                        Nb_pas_de_temps);
