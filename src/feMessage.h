...
#define feErrorMsg(status, ...)     feMessageError(status, __func__, __FILE__, STRINGIFY(__LINE__), ## __VA_ARGS__ )
#define feError(status)             feErrorMsg(status,NULL)

/* give trace message if status is not OK, but does not return */
#define FE_TRACE_MSG(status, ...)    feMessageTraceError(status, __func__, __FILE__, STRINGIFY(__LINE__), ## __VA_ARGS__ )
