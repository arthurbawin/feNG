...
public:
    ...
    for(int i = 0; i < _nElm; ++i){
      for(int j = 0; j < _maxDOFperElem; ++j){
        if(_codeDOFElements[i] == ESS)
          printf("elm %d - dof %d : %d \t %s\n", i, j, _numberingElements[_maxDOFperElem*i+j], "ESS");
        else if(_codeDOFElements[i] == INC)
          printf("elm %d - dof %d : %d \t %s\n", i, j, _numberingElements[_maxDOFperElem*i+j], "INC");
        else
          printf("elm %d - dof %d : %d \t %d\n", i, j, _numberingElements[_maxDOFperElem*i+j], _codeDOFElements[i]);
      }
    }
  }
  ...
  void printNumberingEdges(){
   ...
   void printCodeVertices(){
    for(auto val : _codeDOFVertices)
      std::cout<<val<<std::endl;
  }

  void printCodeElements(){
    for(auto val : _codeDOFElements)
      std::cout<<val<<std::endl;
  }

  void printCodeEdges(){
    for(auto val : _codeDOFEdges)
      std::cout<<val<<std::endl;
  }
