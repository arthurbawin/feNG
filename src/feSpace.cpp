...
feStatus createFiniteElementSpace(feSpace *&space, feMesh *mesh, int dim, elemType type, int deg, std::string fieldID, std::string cncGeoID, int dQuad, feFunction *fct, bool useGlobalShapeFunctions){
  if(mesh == nullptr)
  ...
  switch(dim){
    ...
    case 1 :
      if(type == LINE){
        switch(deg){
          case 0 : space = new feSpace1DP0(mesh, fieldID, cncGeoID, fct); break;
          case 1 : space = new feSpace1DP1(mesh, fieldID, cncGeoID, fct); break;
          case 2 : space = new feSpace1DP2(mesh, fieldID, cncGeoID, fct); break;
          case 3 : space = new feSpace1DP3(mesh, fieldID, cncGeoID, fct); break;
          case 4 : space = new feSpace1DP4(mesh, fieldID, cncGeoID, fct); break;
          default :
            return feErrorMsg(FE_STATUS_ERROR, "No LINE finite element space implemented for deg > 4.");
        }
      } else{
        return feErrorMsg(FE_STATUS_ERROR, "Unsupported geometry.");
      }
    ...
    case 2 :
      if(type == TRI){
        switch(deg){
          case 0 : 
            return feErrorMsg(FE_STATUS_ERROR, "No TRI finite element space implemented for deg > 4.");
          case 1 : space = new feSpaceTriP1(mesh, fieldID, cncGeoID, fct); break;
          case 2 : space = new feSpaceTriP2(mesh, fieldID, cncGeoID, fct); break;
          case 3 : space = new feSpaceTriP3(mesh, fieldID, cncGeoID, fct); break;
          case 4 : space = new feSpaceTriP4(mesh, fieldID, cncGeoID, fct); break;
          default :
            return feErrorMsg(FE_STATUS_ERROR, "No LINE finite element space implemented for deg > 4.");
        }
      } else{
    ...
    default :
      return feErrorMsg(FE_STATUS_ERROR, "Cannot create a finite element space for dimension > 3 or < 0.");
  }
    ...
    _nFunctions(0), _fct(fct), _useGlobalShapeFunctions(useGlobalShapeFunctions)
{
  if(mesh != nullptr) {
    ...
    if(_cncGeoTag == -1){
      feErrorMsg(FE_STATUS_ERROR, "Geometric connectivity \"%s\" does not exist in the mesh.", cncGeoID.c_str());
      std::exit(1);
    }
  }
}
    ...
    switch(this->getPolynomialDegree()){
      case 2 :
      {
        ...
        for(int k = 0; k < _nQuad; ++k){
          for(int i = 0; i < n; ++i){
            for(int j = 0; j < n; ++j){
              m(i,j) += pow(_xQuad[k],ex[i]) * pow(_yQuad[k],ey[j]) * J[iElm * _nQuad + k] * _wQuad[k];
            }
          }
        }
...
double feSpace::interpolateSolution(feNumber *number, feSolution *sol, std::vector<double> &x){
  ...
  if(!isFound){
    printf("In feSpace::interpolateSolution : Warning - Point (%f, %f, %f) was not found in the mesh.\n", x[0], x[1], x[2]);
    return 0.0;
  } else{
...
void feSpace::interpolateSolution_gradrs(feNumber *number, feSolution *sol, std::vector<double> &x, std::vector<double> &grad){
  std::vector<double> u(3,0.0);
  int elm = -1;
  bool isFound = static_cast<feMesh2DP1*>(_mesh)->locateVertex(x,elm,u);
  if(!isFound){
    ...
    this->initializeAddressingVector(number, elm);
    this->initializeSolution(sol);
    grad[0] = this->interpolateSolution_rDerivative(u.data()); // Attention : fonctions de forme locales
    grad[0] = this->interpolateSolution_sDerivative(u.data()); // Attention : fonctions de forme locales
  }
}
