...
void feSysElm_1D_weakBC_edo1::computeAe(feBilinearForm *form)
{
  ...
  for(int k = 0; k < nG; ++k) {
    ...
    for(int i = 0; i < nFunctionsL; ++i) {
      for(int j = 0; j < nFunctionsU; ++j) {
        form->_Ae[nFunctionsU + i][(nFunctionsU + nFunctionsL) + j] +=
          -_feL[i] * _feV[j] * jac * w[k];
      }
    }
...
void feSysElm_0D_EulerFlux::computeBe(feBilinearForm *form)
{
  ...
  if(_boundaryType == 1) {
    ...
    double rhouA_r =
      form->_intSpaces[_idrhouA_dom]->interpolateField(form->_solNext[_idrhouA_dom], rRight);
    ...
    double rhouA_r =
      form->_intSpaces[_idrhouA_dom]->interpolateField(form->_solPrev[_idrhouA_dom], rRight);
