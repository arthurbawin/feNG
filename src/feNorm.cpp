  ...
  for(const feSpace *fS : spaces) {
    if(fS->getCncGeoTag() != cncGeoTag)
      return feErrorMsg(FE_STATUS_ERROR,
                        ...
                        "\t Finite element space for field \"%s\" is defined on connectivity \"%s\"\n"
                        "\t Finite element space for field \"%s\" is defined on connectivity \"%s\"\n"
                        "All finite element spaces in \"spaces\" should be defined on the same connectivity.\n",
                        spaces[0]->getFieldID().data(), spaces[0]->getCncGeoID().data(),
                        fS->getFieldID().data(), fS->getCncGeoID().data());
  }
...
feStatus feNorm::computeErrorNormFromExternalSolution(feMesh *referenceMesh,
                                                      ...
                                                      std::vector<double> &res)
{
  ...
  if(fabs(currentSolution->getCurrentTime() - referenceSolution->getCurrentTime()) > maxTimeDifference) {
    return feErrorMsg(FE_STATUS_ERROR, "Solutions time differ by more than %1.3e :\n"
              "Time of other (reference) solution = %+-1.4e\n"
              "Time of current           solution = %+-1.4e\n",
              maxTimeDifference,
              referenceSolution->getCurrentTime(), currentSolution->getCurrentTime());
  ...
  for(const auto &s : _spaces)
  {
    ...
    if(!spaceWasFound)
      return feErrorMsg(FE_STATUS_ERROR, "Could not compute error from external solution!\n"
        " Current finite element space for field \"%s\""
        " on connectivity \"%s\" could not be found in the given reference vector of spaces.",
  ...
  for(size_t iSpace = 0; iSpace < _spaces.size(); ++iSpace)
  {
    ...
    for(int iElm = 0; iElm < _nElm; ++iElm)
    {
      ...
      for(int k = 0; k < _nQuad; ++k)
      {
        ...
        bool isFound = static_cast<feMesh2DP1*>(referenceMesh)->locateVertex(pos.data(), elm, xsi);
        ...
        if(!isFound) {
          ...
          // Error
          L2error += (uh - uhRef) * (uh - uhRef) * _J[_nQuad * iElm + k] * _w[k];
        }
      }
    }
