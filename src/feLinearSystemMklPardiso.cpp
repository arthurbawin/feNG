...
void feLinearSystemMklPardiso::assignResidualToDCResidual(feSolutionContainer *solContainer){
	for(feInt i = 0; i < matrixOrder; ++i)
    	solContainer->_fResidual[0][i] = residu[i];
};
...
feLinearSystemMklPardiso::feLinearSystemMklPardiso(std::vector<feBilinearForm*> &formMatrices, std::vector<feBilinearForm*> &formResiduals,
...
void feLinearSystemMklPardiso::setPivot(int pivot)
{
	IPARM[9]=(feMKLPardisoInt) pivot;
};
